...
@section('content')
	...
	<!-- end breadcrumb -->
	<!-- begin page-header -->
	<h1 class="page-header">添加 <small></small></h1>
	<!-- end page-header -->
    <!-- begin panel -->
			<div class="panel panel-success" data-sortable-id="form-validation-1">
				...
				<div class="panel-body">
					...
					@endif
					<form class="form-horizontal" action="{{ route('types.store') }}" method="post" data-parsley-validate="true" name="store-form" enctype="multipart/form-data">
                        @csrf
						<div class="form-group row m-b-15">
							<label class="col-md-2 col-sm-2 col-form-label" for="title">名称: <code>*</code> </label>
							<div class="col-md-10 col-sm-10">
								<input class="form-control" type="text" id="title" name="title" value="{{ old('title') }}" placeholder="名称（必填）" data-parsley-required="true" />
							</div>
						</div>
						<div class="form-group row m-b-15">
							<label class="col-md-2 col-sm-2 col-form-label" for="parent_id">分类: <code>*</code> </label>
							<div class="col-md-10 col-sm-10">
                                <select multiple class="form-control" id="parent_id" name="parent_id" data-parsley-required="true" data-parsley-type="number">
									<option value="">分类（必填）</option>
									@foreach ($primaryTypes as $primaryType)  
										<option value="{{ $primaryType->id }}">{{ $primaryType->title }}</option>
										@if ( $primaryType->children->isNotEmpty() && $primaryType->id != 1 )  
											@foreach ($primaryType->children as $secondaryMenu)  
												<option value="{{ $secondaryMenu->id }}"> - {{ $primaryType->title }} - {{ $secondaryMenu->title }}</option>
											@endforeach   
										@endif  
									@endforeach  
								</select>
