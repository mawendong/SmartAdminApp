...
@section('content')
	...
	<!-- end breadcrumb -->
	<!-- begin page-header -->
	<h1 class="page-header">文章列表 <small></small></h1>
	<!-- end page-header -->
	<!-- begin panel -->
				...
				@if ($articles && !$articles->isEmpty()) 
					...
					<tr class="gradeA">
						...
						<td>
							...
							<!-- <form action="{{ route('articles.softdelete', $article->id) }}" method="post"> -->
							<!-- <form action="{{ route('articles.forcedelete', $article->id) }}" method="post"> -->
								@csrf
								@method('DELETE')
								<!-- <input type="hidden" name="force" value="true"> -->
								<a class="btn btn-warning btn-sm m-r-5" href="{{ route('articles.show',$article->id) }}">{{ __('View') }}</a>
								<a class="btn btn-primary btn-sm m-r-5" href="{{ route('articles.edit',$article->id) }}">{{ __('Edit') }}</a>
